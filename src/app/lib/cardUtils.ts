 ...
 * @property placedOnTurn - Pedido do usuário, interface de inspeção de carta ("mostre o turno em que a carta foi posicionada"): `state.turn` no instante EXATO em que esta carta física passou a ocupar um slot do campo (principal ou horizontal) pela primeira vez - carimbado em TODO ponto de `gameEngine.ts` que faz uma carta NASCER num slot (PLAY_CARD em todas as suas variantes - numeral/Ás, horizontal, Broto plantado/empilhado, Monstro do Druida, Criogolem do Glacial -, Modo Towers ao formar/reforçar com cartas novas da mão, e as trocas de campo do Mago/Besta que trazem uma carta de FORA do campo pra dentro dele). NUNCA recarimbado quando uma carta já em campo só é reexposta/revelada/movida (erosão de Torre expondo a reserva, Ás transformado em campo sendo revelado, Ilusão Arcana copiando valor, Broto reduzindo o próprio valor) - nesses casos o carimbo original continua valendo, porque a carta não "chegou" de novo agora. `undefined` só deveria acontecer em cenário de teste sintético que monta `Card` à mão sem passar por `gameEngine.ts` - a UI trata esse caso como "desconhecido", nunca quebra.
 *
